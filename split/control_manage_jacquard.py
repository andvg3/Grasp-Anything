...
# Example usage:
def seen_filter_jacquard(file_path):
    idx = file_path.split('/')[-1].split('.')[0]
    idx = "_".join(idx.split('_')[:2])
    return idx in seen_filters

def unseen_filter_jacquard(file_path):
    idx = file_path.split('/')[-1].split('.')[0]
    idx = "_".join(idx.split('_')[:2])
    return idx in unseen_filters

jacquard_path = 'data/jacquard'
jacquard_filter_path = 'split/jacquard'
source_directory = "data/jacquard"
seen_destination_directory = "data/jacquard_seen"
unseen_destination_directory = "data/jacquard_unseen"

with open(os.path.join(jacquard_filter_path, 'seen.obj'), 'rb') as f:
    seen_filters = pickle.load(f)

with open(os.path.join(jacquard_filter_path, 'unseen.obj'), 'rb') as f:
    unseen_filters = pickle.load(f)

copy_files_with_structure(source_directory, seen_destination_directory, filter_func=seen_filter_jacquard)
copy_files_with_structure(source_directory, unseen_destination_directory, filter_func=unseen_filter_jacquard)
